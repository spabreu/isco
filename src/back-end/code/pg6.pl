...
% CNAME -> class name (Prolog name)
% RNAME -> relation name (SQL name)
...
%%DBG	format('%% ~w\n', [isco_prolog_class_body(Vs, CNAME, RNAME, HEAD, GOAL, CH, OC_VAR+MASK)]), %%DBG
	...
	GOAL = (
	  ...
	  format_to_codes(SQLin, 'select o.oid~s from "~w" o', [SELf, RNAME]),
	  G1 ),
